...
public class BankNiftyOperator extends JPanel{
    ...
    private JPanel createDetailPanel() {
        ...
        getLevelsButton.addActionListener(a -> {
            topLevels = calculateTopLevels33(strikeTF.getText());
            ...
            bottomLevels = calculateBottomLevels33(strikeTF.getText());
            for(int i=0; i<15; i++){
    ...
    private int initialLevel33(int strike){
        int last2no = Integer.parseInt(String.valueOf(strike).substring(3, 5));

        int levelOne = 0;

        if(last2no <= 33){
            levelOne = Integer.parseInt(String.valueOf(strike).substring(0, 3) + "33");
        }
        else{
            int middleNo = Integer.parseInt(String.valueOf(strike).substring(2, 3)) + 1;
            levelOne = Integer.parseInt(String.valueOf(strike).substring(0, 2) + middleNo + "33");
        }

        return levelOne;
    }

    private List<Integer> calculateTopLevels33(String sp) {
        int strikePrice = 0;
        ...
        if(sp.equals("")){
            ...
            res.add(initialLevel33(strikePrice));

            for (int i = 1; i < 15; i++) {
                res.add(initialLevel33(strikePrice) + (i * 100));
            }
        }
        return res;
    }
    
    private int[] calculateBottomLevels33(String sp) {
        int strikePrice = 0;
        ...
        if(sp.equals("")){
            ...
            for (int i = 0; i < 15; i++) {
                res[i] = initialLevel33(strikePrice) - ((i + 1) * 100);
            }
        }
        return res;
    }
